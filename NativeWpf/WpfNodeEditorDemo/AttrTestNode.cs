...
{
    // Keeps the original demo property's enum shape without taking a WinForms dependency.
    ...
    public class AttrTestNode : STNode
    {
        ...
        public static void ShowHelpInfo(string strFileName) {
            System.Windows.MessageBox.Show("this is -> ShowHelpInfo(string);\r\n" + strFileName);
        }
    ...
    public class DescriptorForColor : STNodePropertyDescriptor
    {
        ...
        protected override void OnMouseClick(STNodeMouseEventArgs e) {
            ...
            if (m_rect.Contains(e.Location)) {
                Color current = (Color)this.GetValue(null);
                if (!WpfColorPicker.TryPick(current, out Color selected)) return;
